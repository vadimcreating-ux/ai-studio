...
```
claude/*, feature/* (фича-ветки, от main)
    ↓ разработка готова
main  ──┐
        ↓ merge main + фича-ветка → develop (тестируем переезд)
...
**Правила:**
- Фича-ветки создаются от `main`
- Разработка идёт в фича-ветке (`claude/...`, `feature/...`)
- Перед релизом: мержим `main` в `develop`, затем фича-ветку в `develop`
- На staging проверяем что мерж прошёл чисто и всё работает с реальной схемой БД
...
// ❌ max_tokens — НЕ параметр KIE, не отправлять
{
...
> - **❌ `max_tokens`** — не параметр KIE для Claude, **не отправлять** (вызывает ошибки сервера).
...
claude:  [{ value: "claude-opus-4-6", label: "Claude Opus 4.6" }, { value: "claude-opus-4-5", label: "Claude Opus 4.5" }, { value: "claude-sonnet-4-5", label: "Claude Sonnet 4.5" }, { value: "claude-haiku-4-5", label: "Claude Haiku 4.5" }]
