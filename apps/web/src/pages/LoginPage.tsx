...
const ERROR_MESSAGES: Record<string, string> = {
  google_denied: "Авторизация через Google отменена",
  google_token_failed: "Ошибка получения токена от Google",
...
export default function LoginPage() {
  ...
  const [error, setError] = useState<string | null>(
    searchParams.get("error") ? (ERROR_MESSAGES[searchParams.get("error")!] ?? "Ошибка авторизации") : null
  );
    ...
    <div className="min-h-screen bg-base flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-muted text-sm">Войдите в аккаунт</p>
        </div>
