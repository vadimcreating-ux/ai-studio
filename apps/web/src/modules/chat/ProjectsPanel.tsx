...
export default function ProjectsPanel({
  ...
  const createProject = useMutation({
    mutationFn: (name: string) =>
      projectsApi.create({ module: engine, name }),
    onSuccess: (data) => {
  ...
  const deleteProject = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["projects", engine] });
      if (selectedProjectId) onSelectProject(null);
    },
  });
  ...
  return (
    <div className="flex flex-col w-[280px] min-w-[280px] h-full bg-panel border-r border-border overflow-y-auto scrollbar-thin">
      ...
      {/* Divider + Dialogs */}
      <div className="px-4 pt-4 pb-2 flex items-center justify-between border-t border-border mt-2">
        <span className="text-[13px] font-semibold text-white">Диалоги</span>
        <button
        ...
        {chats.map((chat) => (
          <ChatItem
            ...
            onDelete={() => {
              ...
              if (selectedChatId === chat.id) onNewChat();
            }}
          />
        ))}
        {chats.length === 0 && (
          <div className="px-4 py-3 text-[12px] text-muted leading-snug">
            {selectedProjectId
              ? "Нет диалогов в этом проекте. Нажмите «Чат»."
              : "Нет диалогов. Нажмите «Чат» чтобы начать."}
          </div>
        )}
        {!selectedProjectId && chats.length === 0 && (
          <p className="px-4 text-[11px] text-muted/70 leading-snug mt-1">
...
function ProjectItem({
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative px-4 py-2.5 cursor-pointer border-l-2 transition-colors ${
        isActive
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div
            className={`text-[13px] font-medium truncate ${
              isActive ? "text-white" : "text-[#c9d1d9]"
            }`}
          >
            {project.name}
          </div>
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          <button
            ...
            className="p-0.5 rounded hover:text-red-400 text-muted"
          >
            <Trash2 size={12} />
          </button>
        </div>
...
function ChatItem({
  ...
  return (
    <div
      onClick={onClick}
      className={`group px-4 py-2.5 cursor-pointer border-l-2 transition-colors ${
        isActive
          ? "border-accent bg-accent-subtle"
          : "border-transparent hover:bg-surface"
      }`}
