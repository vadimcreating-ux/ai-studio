...
export async function imageRoutes(app) {
    app.post("/api/image/generate", async (request, reply) => {
        ...
        try {
            const createResponse = await fetch(`${KIE_BASE_URL}/api/v1/flux/kontext/generate`, {
                method: "POST",
                headers: {
    ...
    app.get("/api/image/status", async (request, reply) => {
        ...
        try {
            const statusResponse = await fetch(`${KIE_BASE_URL}/api/v1/flux/kontext/record-info?taskId=${encodeURIComponent(taskId)}`, {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${apiKey}`,
                },
    ...
    app.get("/api/image/download", async (request, reply) => {
        ...
        const fileName = (query?.name?.trim() || "generated-image.jpg").replace(/[^a-zA-Z0-9._-]/g, "_");
        if (!fileUrl) {
            return reply.status(400).send({
                ok: false,
