...
import { AdminAddCreditsSchema, AdminUpdateUserSchema, AdminUpdateCreditPriceSchema, AdminUpdateStorageSchema, AdminCreateUserSchema, AdminResetPasswordSchema, } from "../lib/validation.js";
export async function adminRoutes(app) {
    ...
    app.get("/api/admin/users", async (_req, reply) => {
        const result = await dbQuery("SELECT id, email, name, role, is_active, credits_balance, storage_quota_mb, storage_used_mb, created_at FROM users ORDER BY created_at DESC");
    ...
    app.post("/api/admin/users", async (req, reply) => {
        ...
        const result = await dbQuery("INSERT INTO users (email, password_hash, name, role, credits_balance) VALUES ($1, $2, $3, $4, $5) RETURNING id, email, name, role, is_active, credits_balance, storage_quota_mb, storage_used_mb, created_at", [email.toLowerCase(), passwordHash, name, role, credits_balance]);
        return reply.status(201).send({ ok: true, data: result.rows[0] });
    });
    // GET /api/admin/users/:id
    app.get("/api/admin/users/:id", async (req, reply) => {
        const { id } = req.params;
        const result = await dbQuery("SELECT id, email, name, role, is_active, credits_balance, storage_quota_mb, storage_used_mb, created_at FROM users WHERE id = $1", [id]);
        if (!result.rows[0])
    ...
    app.patch("/api/admin/users/:id", async (req, reply) => {
        ...
        values.push(id);
        const result = await dbQuery(`UPDATE users SET ${fields.join(", ")} WHERE id = $${i} RETURNING id, email, name, role, is_active, credits_balance, storage_quota_mb, storage_used_mb, created_at`, values);
        if (!result.rows[0])
            return reply.status(404).send({ ok: false, error: "Пользователь не найден" });
        return reply.send({ ok: true, data: result.rows[0] });
    });
    ...
    app.patch("/api/admin/users/:id/password", async (req, reply) => {
        ...
        const result = await dbQuery("UPDATE users SET password_hash = $1, jwt_version = jwt_version + 1 WHERE id = $2 RETURNING id", [newHash, id]);
        if (!result.rows[0])
            return reply.status(404).send({ ok: false, error: "Пользователь не найден" });
        return reply.send({ ok: true, data: null });
    });
    ...
    app.post("/api/admin/users/:id/credits", async (req, reply) => {
        ...
        await dbQuery("UPDATE users SET credits_balance = credits_balance + $1 WHERE id = $2", [amount, id]);
        await dbQuery("INSERT INTO credit_transactions (user_id, amount, type, operation, description) VALUES ($1, $2, 'admin_add', 'admin_add', $3)", [id, amount, description ?? `Начисление от администратора (${adminUser.email})`]);
    ...
    app.patch("/api/admin/users/:id/storage", async (req, reply) => {
        ...
        const result = await dbQuery("UPDATE users SET storage_quota_mb = $1 WHERE id = $2 RETURNING id, storage_quota_mb, storage_used_mb", [storage_quota_mb, id]);
        if (!result.rows[0])
            return reply.status(404).send({ ok: false, error: "Пользователь не найден" });
        return reply.send({ ok: true, data: result.rows[0] });
    });
    ...
    app.put("/api/admin/credit-prices/:operation", async (req, reply) => {
        ...
        await dbQuery(`INSERT INTO credit_prices (operation, credits, markup_percent) VALUES ($1, $2, $3)
       ON CONFLICT (operation) DO UPDATE SET credits = $2, markup_percent = $3`, [operation, credits, markup_percent]);
    ...
    app.get("/api/admin/transactions", async (req, reply) => {
        ...
        const result = await dbQuery(`SELECT ct.id, ct.user_id, u.email, u.name, ct.amount, ct.type, ct.operation, ct.description, ct.created_at
       FROM credit_transactions ct
       JOIN users u ON u.id = ct.user_id
       ${where}
       ORDER BY ct.created_at DESC
       LIMIT $${params.length - 1} OFFSET $${params.length}`, params);
        const total = await dbQuery(`SELECT COUNT(*) as count FROM credit_transactions ct ${where}`, whereParts.length ? params.slice(0, -2) : []);
