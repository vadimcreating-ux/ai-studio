...
export async function engineSettingsRoutes(app) {
    ...
    app.get("/api/engine-settings/:engine", async (request, reply) => {
        ...
        const result = await dbQuery(`SELECT * FROM engine_settings WHERE engine = $1`, [engine]);
    ...
    app.put("/api/engine-settings/:engine", { preHandler: [requireAdmin] }, async (request, reply) => {
        ...
        await dbQuery(`INSERT INTO engine_settings (engine, about, instructions, memory, updated_at)
       VALUES ($1, $2, $3, $4, now())
       ON CONFLICT (engine) DO UPDATE
         SET about = EXCLUDED.about,
             instructions = EXCLUDED.instructions,
             memory = EXCLUDED.memory,
             updated_at = now()`, [engine, about, instructions, memory]);
        return { ok: true, settings: { engine, about, instructions, memory } };
    });
}
