...
export async function creditsRoutes(app) {
    ...
    app.get("/api/credits/history", async (req, reply) => {
        ...
        const [rows, total] = await Promise.all([
            dbQuery("SELECT id, amount, type, operation, description, created_at FROM credit_transactions WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3", [user.userId, limit, offset]),
