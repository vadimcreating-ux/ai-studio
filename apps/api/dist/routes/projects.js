...
export async function projectRoutes(app) {
    ...
    app.get("/api/projects", async (request) => {
        ...
        const result = user.role === "admin"
            ? await dbQuery(`SELECT * FROM projects WHERE module = $1 ORDER BY created_at ASC`, [module])
            : await dbQuery(`SELECT * FROM projects WHERE module = $1 AND (user_id = $2 OR user_id IS NULL) ORDER BY created_at ASC`, [module, user.userId]);
    ...
    app.put("/api/projects/:id", async (request, reply) => {
        ...
        const result = user.role === "admin"
           ...
           WHERE id = $8 RETURNING *`, updateParams)
            : await dbQuery(`UPDATE projects
           SET
             name = COALESCE($1, name),
             description = COALESCE($2, description),
             ...
             memory = COALESCE($6, memory),
             context_files = COALESCE($7, context_files)
           WHERE id = $8 AND (user_id = $9 OR user_id IS NULL) RETURNING *`, [...updateParams, user.userId]);
