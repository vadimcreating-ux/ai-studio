...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/api/auth/register", async (req, reply) => {
    ...
    const result = await dbQuery(
      "INSERT INTO users (email, password_hash, name, role) VALUES ($1, $2, $3, $4) RETURNING id, email, name, role, credits_balance",
  ...
  app.post("/api/auth/login", async (req, reply) => {
    ...
    const result = await dbQuery(
      "SELECT id, email, name, role, password_hash, is_active, credits_balance FROM users WHERE email = $1",
      [email.toLowerCase()]
    );
    const user = result.rows[0];
  ...
  app.get("/api/auth/google/callback", async (req, reply) => {
    ...
    try {
      ...
      const googleUser = await userRes.json() as { sub?: string; email?: string; name?: string; email_verified?: boolean };
      ...
      if (!user) {
        ...
        if (user) {
          ...
          const role = isFirstUser ? "admin" : "user";
          const result = await dbQuery(
            "INSERT INTO users (email, google_id, name, role) VALUES ($1, $2, $3, $4) RETURNING id, email, name, role, is_active, credits_balance",
            [googleUser.email.toLowerCase(), googleUser.sub, googleUser.name ?? googleUser.email, role]
          );
          user = result.rows[0];
        }
      }
